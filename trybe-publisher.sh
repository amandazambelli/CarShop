...
if [[ " ${REPO_LIST[*]} " =~ " ${REPO_NAME} " ]]; then
    echo
    echo -e "${RED}O nome ${NO_COLOR}'$REPO_NAME'${RED} escolhido para o novo repositório já está em uso."
    echo
    exit 1
fi
...
if [[ ! $BRANCHES ]] ; then
    echo -e "${RED}O valor informado no parâmetro ${RED}-b${NO_COLOR} não corresponde a uma branch deste repositório.${NO_COLOR}"
    exit 1
fi
...
if [[ ! -d $SCRIPTS_BASE ]]; then
    echo -e "${CYAN}Obtendo base configurações de limpeza${NO_COLOR}"
    ...
    echo -e "${CYAN}Atualizando base configurações de limpeza${NO_COLOR}"
    CURR_PATH=$(pwd)
...
if [[ $(find . -type f -name trybe-filter-repo.sh | head -n 1) ]] ; then
    echo -e "${CYAN}A configuração de limpeza para este projeto está disponível!${NO_COLOR}"
else
    if [[ $(find $SCRIPTS_BASE/repo-filters-by-project/ -type d -name "*$current_project" | head -n 1) ]] ; then
        echo -e "${CYAN}A configuração de limpeza para este projeto foi encontrada no repositório base!${NO_COLOR}"
        cp $SCRIPTS_BASE/repo-filters-by-project/*$current_project/trybe-filter-repo.sh ./
    else
        echo -e "${BR}${YELLOW}Não foi encontrada uma configuração de limpeza para o projeto ${current_project}!${NO_COLOR}${BR}"
...
if ! git fetch origin --quiet ; then # não tem acesso ou porque não existe ou porque não tem autorização
    echo -e "${CYAN}A ferramenta usará os arquivos ${NO_COLOR}locais${CYAN}."
    echo -e "Obs: isso acontece porque esse repositório não existe ou você não tem autorização para acessá-lo mais.${NO_COLOR}${BR}"
else
    ...
    if [[ $HAS_REMOTE_MODS ]]; then
        ...
        if [[ $REPLY =~ ^[Ss]$ ]]; then
            ...
            echo 
            echo -e "${BR}${CYAN}Tudo bem, a ferramenta usará os arquivos ${NO_COLOR}locais${CYAN}.${NO_COLOR}${BR}"
        fi
    fi
fi
...
if git push -u $REMOTE_NAME $TARGET_BRANCH:main ; then
    echo -e "${GREEN}> Operação de push feita com sucesso!${NO_COLOR}"
else
...
then 
    ...
    echo -e "${GREEN}> O remote ${NO_COLOR}origin${GREEN} foi atualizado para o novo repositório."
fi
